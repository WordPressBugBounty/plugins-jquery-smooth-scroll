...
if ( ! class_exists( 'Jamify_Jquery_Smooth_Scroll' ) ) {

	/**
	 * Main Class Jamify_Jquery_Smooth_Scroll
	 */
	class Jamify_Jquery_Smooth_Scroll {
		...
		public function page_init() {
			// Migration Logic: Check if old option exists and new one doesn't.
			if ( get_option( 'jss_scroll_to_top_image' ) && ! get_option( 'jamify_jquery_smooth_scroll_image' ) ) {
				update_option( 'jamify_jquery_smooth_scroll_image', get_option( 'jss_scroll_to_top_image' ) );
				delete_option( 'jss_scroll_to_top_image' );
			}

			register_setting( 'jquery_smooth_scroll_option_group', 'jamify_jquery_smooth_scroll_image', 'esc_url_raw' );
		}
				...
				<form method="post" action="options.php">
					...
					<table class="form-table">
						<tr valign="top">
							...
							<td>
								<?php $image_url = get_option( 'jamify_jquery_smooth_scroll_image' ); ?>
								<input type="hidden" id="jamify_jquery_smooth_scroll_image" name="jamify_jquery_smooth_scroll_image" value="<?php echo esc_attr( $image_url ); ?>" />
		...
		/**
		 * Load CSS and Scripts to the head
		 */
		...
		/**
		 * Add button to wp_footer
		 */
		public function wp_footer() {
			// the html button which will be added to wp_footer
			$image_url = get_option( 'jamify_jquery_smooth_scroll_image' );
			if ( $image_url ) {
				?>
	...
	$jamify_jquery_smooth_scroll = new Jamify_Jquery_Smooth_Scroll();

	// Backward compatibility alias
	if ( ! class_exists( 'jQuerySmoothScroll' ) ) {
		class_alias( 'Jamify_Jquery_Smooth_Scroll', 'jQuerySmoothScroll' );
	}
